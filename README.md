...
The "GalileoTestFW"- automation test framework covers backend and frontend, provides functionality testing:
- Unit
- API
- UI
- Integration
...
- https://gitlab.boquar.tech/qa-qc/galileotest
...
$ pytest -v tests --alluredir=src/allure__/allure_results

2. Run tests as a package:

$ pytest -v tests/platform_tests --alluredir=src/allure_/allure_results

3. Run specific test:

$ pytest -v tests/platform_tests/get_entities_data_test.py  --alluredir=src/allure_/allure_results

4. Run per test group (regression group as example):

$ pytest -v tests -m regression --alluredir=src/allure_/allure_results

5. Generate temporary allure report:

$ allure.bat serve src/allure_/allure_results

6. Generate report:

$ allure.bat generate src/allure_/allure_results -o src/allure_/allure_report --clean

7. Open allure report:

$ allure.bat open src/allure_/allure_report
...
$ ENV=int tox -- -m [GROUP_NAME] --alluredir=../src/allure_/allure_results

11. Run tests under tox:

$ tox -- -m [GROUP_NAME] --alluredir=../src/allure_/allure_results
...
4. negative - negative tests include ddt (currently ddt tests are skipped)
5. regression - all api tests


CONFIGURATION
--------------

- Project base configuration stores in config.cfg that processes by config.py class.
...
$ docker build --build-arg http_proxy=http://144.72.255.21:80 . --rm -f "Dockerfile" -t galileo_test:latest

3/1) Run tox

$ HTTP_PROXY=http://144.72.225.21:80 tox -- -m smoke --alluredir=../src/allure_/allure_results
...
* Anton Shipulin <shipulin.a@galileosky.ru> 
